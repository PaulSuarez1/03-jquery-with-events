...
articleView.handleMainNav = function () {
  ...
  $('nav').on('click', '.tab', function (event) {
    ...
    if ($(this).val()) {
      $('article').hide();
      // $(`article[tab-content=${$(this).val()}]`).fadeIn('500');
      $('article[tab-content =' + $(this).val() + ']').fadeIn('500');
    } else {
      $('article').show();
      // .hasClass('template')
...
$(document).ready(function () {
  ...
  articleView.handleAuthorFilter(); //Not working//
  articleView.handleCategoryFilter();
  articleView.handleMainNav(); //Not working//
  articleView.setTeasers(); //Not working//
})
